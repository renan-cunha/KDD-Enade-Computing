...
NUM_DIS_GEN_QUESTIONS = 3
NUM_OBJ_GEN_QUESTIONS = 7
NUM_DIS_SPE_QUESTIONS = 2
NUM_OBJ_SPE_QUESTIONS = 28
...
def get_processed_enade_2005(path_csv: str) -> Tuple[pd.DataFrame,
                                                     pd.DataFrame]:
...
def get_discursive_scores(df: pd.DataFrame, general: bool) -> pd.DataFrame:
    ...
    general_indexes = [8, 9, 10]
    specific_indexes = [25, 40]
...
def get_objective_scores(df: pd.DataFrame, general: bool) -> pd.DataFrame:
    ...
    general_indexes = list(range(1, 7 + 1))
    specific_indexes = list(range(11, 24 + 1)) + list(range(26, 39 + 1))
    label = "fg" if general else "ce"
    indexes = general_indexes if general else specific_indexes
    for question_index, i in enumerate(indexes):
        ...
        if i > 10: # this exam has more questions
            question_index += 28
