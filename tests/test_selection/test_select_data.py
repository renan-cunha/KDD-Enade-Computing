...
class TestFilter2005:
    ...
    def test_example(self) -> None:
        ...
        input_dataframe = pd.DataFrame({"co_curso": [config.UFPA_CODE_COURSE,
                                                     config.UFPA_CODE_COURSE,
                                                     1245, 1256, 1257],
                                        "vt_ace_oce": [".abc..", ".ab...",
                                                       ".acc..",
                                                       ".adec.",
        ...
        expected_df = pd.DataFrame({"co_curso": [config.UFPA_CODE_COURSE,
                                                 config.UFPA_CODE_COURSE,
                                                 1245],
                                    "vt_ace_oce": [".abc..", ".ab...",
                                                  ".acc.."],
...
def test_make_select_data_shape(year: int, label: str, expected: int) -> None:
    df = select_data.read_csv(year)
    df_student_present = df.loc[df[label].isin([555, 556])]
